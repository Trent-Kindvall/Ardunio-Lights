/*
	EASYIOT LIBRARY TO SEND TO CLOUD
...
void SNTP(){
	Serial.print("Configurando a hora utilizando servidor SNTP -> ");
...
String send(String Dados, String Token) {
	...
	String resposta = "";
	// Carrega o certificado CA do site
    bool res = client.setCACert_P(caCert, caCertLen);
    if (!res) {
    	Serial.println("Falha ao validar o certificado CA!");
    	while (true) {
        	yield();
        }
    }
	...
	if (!client.connect(host, httpsPort)) {
		...
		Serial.println();
		resposta = "Conexao Falhou";
	    return resposta;
	}

	// Verificar a validada do certificado do servidor
	if (client.verifyCertChain(host)) {
		Serial.println("Certificado do servidor verificado com sucesso.");
		Serial.println();
	}
	else {
		Serial.println("ERRO: Verificacao do servidor falhou!");
		Serial.println();
	}
